...
- Radioisotope and cosmic-ray source energy distributions (can the implemented in Python).
- Some of the grouped row-like data don't support optional arguments (e.g. =non= in the input) because PHITS requires that at least one of the rows in any given column be non-skipped, which is harder to implement checks for than it's worth.
...
Importing semantic explanations from the PHITS documentation into the Python documentation somehow would be nice.
